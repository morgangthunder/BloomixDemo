...
import { ArrowLeftIcon, ChevronDoubleLeftIcon, ChevronDownIcon, ChevronRightIcon, DocumentTextIcon, PlusCircleIcon, TrashIcon, GripVerticalIcon } from './Icon';
...
const StageNavItem: React.FC<{
    ...
    onDelete: (item: SelectedItem) => void;
    onReorderSubStage: (stageId: number, draggedId: number, targetId: number) => void;
}> = ({ stage, selectedItem, onSelectItem, onAddSubStage, onDelete, onReorderSubStage }) => {
...
const SubStageNavItem: React.FC<{
    ...
    onSelectItem: (item: SelectedItem) => void;
    onDelete: (item: SelectedItem) => void;
    onReorder: (stageId: number, draggedId: number, targetId: number) => void;
}> = ({ subStage, stageId, selectedItem, onSelectItem, onDelete, onReorder }) => {
    const isSubStageSelected = selectedItem?.type === 'substage' && selectedItem.id === subStage.id;
    const [isDraggedOver, setIsDraggedOver] = useState(false);

    const handleDragStart = (e: React.DragEvent) => {
        e.dataTransfer.setData('application/json', JSON.stringify({ subStageId: subStage.id, stageId: stageId }));
    ...
    const handleDragEnter = (e: React.DragEvent) => {
        ...
        const { stageId: fromStageId, subStageId: draggedId } = JSON.parse(transferData);

        if (fromStageId === stageId && draggedId !== subStage.id) {
    ...
    const handleDrop = (e: React.DragEvent) => {
        ...
        const { subStageId: draggedSubStageId, stageId: fromStageId } = JSON.parse(transferData);

        if (fromStageId === stageId && draggedSubStageId !== subStage.id) {
            onReorder(stageId, draggedSubStageId, subStage.id);
        }
    };
...
const LessonBuilderNav: React.FC<LessonBuilderNavProps> = ({ lesson, selectedItem, onSelectItem, onShowScript, onAddStage, onAddSubStage, onDelete, onReorderSubStage, onExit, onToggleCollapse }) => {
    ...
    <div className="h-full flex flex-col p-4 overflow-hidden">
            ...
            <div className="flex justify-between items-center">
                ...
                <div className="flex items-center space-x-2">
                    <button 
                        ...
                        className={`p-2 rounded-md transition-colors ${canShowScript ? 'text-brand-light-gray hover:bg-gray-700' : 'text-gray-600 cursor-not-allowed'}`}
                        title="View Script"
                    >
                    ...
                    <button 
                        ...
                        className="p-2 rounded-md text-brand-light-gray hover:bg-gray-700 transition-colors"
        ...
        <nav className="flex-1 overflow-y-auto mt-4 space-y-2 pr-2 scrollbar-thin scrollbar-thumb-gray-700">
            <NavItem 
