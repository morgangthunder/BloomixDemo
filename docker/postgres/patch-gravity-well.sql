-- Gravity Well interaction type
-- Match floating concept orbs to reveal a hidden image behind dissolving fog.
-- Run: docker compose exec -T postgres psql -U upora_user -d upora_dev -f /docker-entrypoint-initdb.d/../patch-gravity-well.sql
...
INSERT INTO interaction_types (
  ...
  'gravity-well',
  'Gravity Well',
  'Match floating concept orbs to reveal a hidden image. Orbs drift in zero-gravity space — click one to pull it into the central gravity well, then click its match. Correct pairs fuse, burn away fog, and reveal part of the target image. Includes SDK, personalisation pipeline, responsive viewport, pause system, background music, constellation finish, and results popup.',
  'explore',
  '{}',
  'Provide pairs of related concepts and an image description. JSON structure: { "pairs": [{"a":"Term","b":"Definition"},...], "imageDescription": "...", "subject": "..." }',
  'pixijs',
  ...
  <div id="input-section" style="display:none">
    <div class="input-card">
      <h3 id="topic-label">Gravity Well</h3>
...
@keyframes shake { 0%,100% { transform: translateX(0); } 25% { transform: translateX(-6px); } 75% { transform: translateX(6px); } }',
  ...
  '{"fields":[{"key":"title","type":"string","label":"Topic Title","description":"Title used for image generation prompts"},{"key":"imageDescription","type":"string","label":"Image Description","multiline":true,"description":"Detailed description for the generated image"},{"key":"pairs","type":"json","label":"Concept Pairs","description":"Array of pairs: [{\"a\":\"Term\",\"b\":\"Definition\"},...]"},{"key":"testMode","type":"boolean","label":"Test Mode","default":true,"description":"When on, shows manual movie/TV input. When off, uses personalisation pipeline."},{"key":"personalisation","type":"boolean","label":"Personalisation","default":true,"description":"When enabled, each user gets a personalised image style. When disabled, all users share the same generated image."},{"key":"bgMusicStyle","type":"string","label":"Background Music Style","default":"ambient","description":"Music style: calm, upbeat, focus, ambient, retro, none"},{"key":"bgMusicVolume","type":"number","label":"Music Volume","default":0.04,"description":"Background music volume 0-1"}]}',
  '{"pairs":[{"a":"H\u2082O","b":"Water"},{"a":"NaCl","b":"Salt"},{"a":"CO\u2082","b":"Carbon Dioxide"},{"a":"O\u2082","b":"Oxygen"},{"a":"Fe","b":"Iron"}],"imageDescription":"A vibrant, richly detailed periodic table of chemical elements with colourful glowing symbols, floating in a cosmic space background with nebulae and stars.","subject":"Chemistry"}',
  '{"fields":[{"name":"time","type":"number","description":"Completion time in seconds"},{"name":"pairs","type":"number","description":"Total pairs matched"},{"name":"wrongAttempts","type":"number","description":"Number of wrong match attempts"}]}',
  '{"customFields":[{"name":"time","type":"number"},{"name":"pairs","type":"number"},{"name":"wrongAttempts","type":"number"}]}',
