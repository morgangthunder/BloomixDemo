        ...
        <div class="modal-body">
          ...
          <div class="form-group" *ngIf="sourceType === 'upload'">
            ...
            <input 
              ...
              class="form-control">
            <small class="help-text">Supported: PDF files only (Max 50MB)</small>
          </div>
          ...
          <div class="alert alert-info">
            ℹ️ PDF upload and text extraction are not yet implemented. Use URL option for now.
          </div>
        </div>
...
export class AddPdfModalComponent implements OnChanges {
  @Input() isOpen = false;
  ...
  canSubmit(): boolean {
    if (this.sourceType === 'url') {
      return this.title.trim().length > 0 && this.pdfUrl.trim().length > 0;
    } else {
  ...
  async onSubmit() {
    ...
    try {
      ...
      if (this.sourceType === 'url') {
        ...
        alert('PDF upload is not yet implemented. Please use URL option for now.');
      ...
      const contentData = {
        type: 'pdf',
        title: this.title,
        summary: this.description || `PDF document: ${this.title}`,
        sourceUrl,
        filePath,
        metadata: {
          topics: topicsArray,
