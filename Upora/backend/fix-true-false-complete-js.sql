-- Complete rewrite of true-false-selection JavaScript code
...
console.log("[Interaction] ⚙️ Initial config check:", getConfig());
...
function initializeWhenReady() {
    ...
    function renderFragments() {
        ...
        if (!data.fragments || data.fragments.length === 0) {
            console.log("[Interaction] ⚠️ No fragments found in data!");
    ...
    function checkAnswers() {
        ...
        scoreBreakdown.innerHTML = `
            <p>You selected ${selectedFragments.size} statements.</p>
            <p>There were ${totalTrue} true statements.</p>
        `;
    ...
    function closeScoreModal() {
        ...
        console.log("[Interaction] ✖️ Score modal hidden");
    }
...
function startInitialization() {
    ...
    const data = getData();
    if (!data.fragments || data.fragments.length === 0) {
        console.log("[Interaction] ⚠️ No data yet, waiting...");
