-- Update SDK Test Media Player interaction to have proper Code tab structure
-- Similar to SDK Test iFrame, but for Media Player (uploaded-media) category
-- This includes HTML, CSS, JS for overlay, config JSON, and processed content selector

UPDATE interaction_types
SET
  -- Media Config JSON (similar to iframe_config for iframe interactions)
  media_config = '{"autoplay": false, "loop": false, "showControls": true, "defaultVolume": 1.0}',
  
  -- HTML Code for overlay (test buttons structure)
  html_code = '<div id="sdk-test-media-overlay">
  ...
  css_code = '#sdk-test-media-overlay {
  position: absolute;
  bottom: 0;
  left: 0;
  right: 0;
...
.result-item.success {
  ...
  -- JavaScript Code for overlay (with all SDK test buttons including media control methods)
  js_code = '// SDK Test Media Player Interaction
// This is an example of how to use overlay mode for Media Player interactions
// The overlay code runs on top of the media player and can use the AI Teacher SDK
...
function initTestApp() {
  ...
  description = 'Comprehensive test interaction for all AI Teacher SDK functionality including media control methods. This interaction uses an overlay at the bottom of the media player to display test buttons. Select a media file in the Settings tab, then view the Code tab to see the overlay HTML/CSS/JS code.'
