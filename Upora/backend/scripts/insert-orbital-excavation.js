// Inserts the orbital-excavation interaction type into the database.
// Run: docker compose exec -T backend node scripts/insert-orbital-excavation.js
...
async function main() {
  ...
  const jsCode = fs.readFileSync(path.join(__dirname, 'orbital-excavation-full-code.js'), 'utf-8');
...
@keyframes shake { 0%,100% { transform: translateX(0); } 25% { transform: translateX(-6px); } 75% { transform: translateX(6px); } }`;
  ...
  console.log('✅ Orbital Excavation interaction type inserted/updated successfully');
  await client.end();
}
