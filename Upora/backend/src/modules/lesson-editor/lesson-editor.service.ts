...
export class LessonEditorService {
  ...
  async getProcessedOutputs(lessonId: string): Promise<any[]> {
    ...
    for (const output of outputs) {
      if (!output.contentSourceId) {
        await this.ensureSourceContentExists(output);
      }
    }
  ...
  /**
   * Ensures that processed content has a corresponding source content.
   * If not, creates a human-readable text source from the interaction JSON.
   */
  private async ensureSourceContentExists(output: ProcessedContentOutput): Promise<void> {
    try {
      const lesson = await this.lessonRepo.findOne({ where: { id: output.lessonId } });
      if (!lesson) return;
      
      // Generate human-readable text from the interaction data
      const sourceText = this.generateSourceTextFromInteraction(output);
      
      // Create the content source
      const contentSource = this.contentSourceRepo.create({
        tenantId: lesson.tenantId,
        type: 'text' as any,
        title: `Source for: ${output.outputName}`,
        summary: `Auto-generated source content for ${output.outputType} interaction`,
        fullText: sourceText,
        status: 'approved' as any,
        createdBy: output.createdBy || undefined,
        metadata: {
          autoGenerated: true,
          interactionType: output.outputType,
          processedOutputId: output.id
        }
      });
      
      const savedSource = await this.contentSourceRepo.save(contentSource);
      
      // Link the processed output to the source
      await this.processedOutputRepo.update(output.id, {
        contentSourceId: savedSource.id as any
      });
      
      this.logger.log(`✅ Auto-generated source content ${savedSource.id} for processed output ${output.id}`);
    } catch (error) {
      this.logger.error(`Failed to auto-generate source content: ${error.message}`);
    }
  }
  
  /**
   * Converts interaction JSON data into human-readable text
   */
  private generateSourceTextFromInteraction(output: ProcessedContentOutput): string {
    const data = output.outputData;
    
    switch (output.outputType) {
      case 'true-false-selection':
        if (data?.statements && Array.isArray(data.statements)) {
          const lines = ['True/False Statements:\n'];
          data.statements.forEach((stmt: any, idx: number) => {
            lines.push(`${idx + 1}. ${stmt.text} - ${stmt.isTrue ? 'TRUE' : 'FALSE'}`);
          });
          return lines.join('\n');
        }
        break;
      
      case 'fragment-builder':
        if (data?.fragments) {
      ...
      default:
        // Generic fallback - prettify the JSON
        return `Interaction Data (${output.outputType}):\n\n${JSON.stringify(data, null, 2)}`;
    }
